...
export function useWindowManager(
  ...
  getAppContent: (
    ...
    owner?: string,
    remoteComputerId?: string,
  ) => { content: React.ReactNode; title: string; windowId?: string },
    ...
    try {
      ...
      if (sessionsRaw && Array.isArray(sessionsRaw)) {
        const sessions: WindowSession[] = sessionsRaw
          .filter(
            (s) => s && typeof s.id === "string" && typeof s.type === "string",
          )
          .map((s) => ({
            id: String(s.id),
            type: String(s.type),
            ...
            data: s.data,
            owner:
              typeof s.owner === "string" ? s.owner : activeUser || "guest",
            remoteComputerId: s.remoteComputerId,
          }));
  ...
  useEffect(() => {
    ...
    const sessions: WindowSession[] = windows.map((w) => ({
      ...
      data: w.data,
      owner: w.owner,
      remoteComputerId: w.remoteComputerId,
    }));
  ...
  const openWindow = useCallback(
    (
      ...
      if (app && app.ramUsage) {
        ...
        const isAppAlreadyOpen = windows.some(
          (w) => w.type === type && w.owner === windowOwner,
        );
        ...
        if (usedMB + cost > totalMemoryMB) {
          notify.system(
            "error",
            t("memory.error.title"),
            t("memory.error.description", { appName: app.name }),
          );
      ...
      setWindows((prevWindows) => {
        ...
        feedback.windowOpen();
        const { content, title, windowId } = getAppContent(
          type,
          data,
          windowOwner,
          remoteComputerId,
        ...
        const maxStepsX = Math.floor(
          (screenW - width - startX - rightPadding) / stepSize,
        );
        const maxStepsY = Math.floor(
          (screenH - height - startY - bottomPadding) / stepSize,
        );
